  ...
  <div class="cp-page">
    <div class="cp-head glass">
      <div class="left">
        ...
        <div class="sub">All return operations</div>
      </div>
    ...
    <div class="cp-card glass">
      <div class="table">
        <div class="thead">
          ...
          <div class="th c-client">Client</div>
          <div class="th c-type">Type</div>
          <div class="th c-amount">Amount</div>
          <div class="th c-method">Method</div>
        ...
        <div v-else v-for="r in rows" :key="r.id" class="tr">
          ...
          <div class="td c-client">
            <div class="client-main">{{ r.client?.login || r.client?.phone || r.client_id }}</div>
            <div class="muted">ID: {{ r.client_id }}</div>
          </div>
          <div class="td c-type">{{ r.type }}</div>
          <div class="td c-amount">{{ money(r.amount) }} UZS</div>
          <div class="td c-method">{{ r.payment_method || '-' }}</div>
...
async function load(p = 1) {
  loading.value = true
  try {
    const r = await cpApi.returnsList({ page: p })
...
.thead, .tr{
  display:grid;
  grid-template-columns: 160px 1.1fr 100px 140px 100px 140px 80px;
  align-items:center;
}
.thead{
